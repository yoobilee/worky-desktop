...
export default function ClientsPage({ user }: { user: User }) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden" style={{ background: 'transparent' }}>
      {toast && (
        <div
          className="fixed top-10 left-1/2 -translate-x-1/2 z-[9999] px-4 py-1.5 rounded-lg text-[11px] font-medium text-center pointer-events-none shadow-lg max-w-[280px] whitespace-normal"
          style={{
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-0.5" style={{ scrollbarWidth: 'none' }} onWheel={(e) => { e.preventDefault(); e.currentTarget.scrollLeft += e.deltaY }}>
              {recentOpens.slice(0, settings.recentMax).map((e) => (
                ...
                >
                  ...
                  <span className="max-w-[60px] truncate" title={e.clientName}>{e.clientName}</span>
                </button>
              ))}
            </div>
          </div>
